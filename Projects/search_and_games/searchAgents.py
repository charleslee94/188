...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        i = 0
        while i<4:
            if self.corners[i] == start:
                cornerCheck[i] = True
            i = i + 1
        return (start, tuple(cornerCheck))
...
def cornersHeuristic(state, problem):
    ...
    while i<4:
        if not cornersReached[i]:
            targetCorners.append(corners[i])
        i = i+1
    total = 0
    trav = state[0]
    while targetCorners:
        i = 0
        distances = []
        while i<len(targetCorners):
            distances.append(util.manhattanDistance(trav, targetCorners[i]))
            i=i+1
        minDistance = min(distances)
        total += minDistance
        closestCorner = targetCorners[distances.index(minDistance)]
        trav = closestCorner
        targetCorners.remove(closestCorner)
...
def foodHeuristic(state, problem):
    ...
    if len(foodlist) == 0: #no food left
        return 0
    mandist = []
    for elem in foodlist:
        mandist.append(util.manhattanDistance((elem[0], elem[1]), position))
    closestfood = foodlist[mandist.index(min(mandist))]
