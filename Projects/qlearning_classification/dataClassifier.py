...
def enhancedFeatureExtractorDigit(datum):
    ...
    search through the grid for pixels and recursively through their neighbors
    ##
    """
...
def visiter(pixel, visited, datum):
    if pixel in visited:
        return
    visited.append(pixel)
    neighbors = [] # Get neighbors
    if (pixel[0] - 1 in range(DIGIT_DATUM_WIDTH) and pixel[1] - 1 in range(DIGIT_DATUM_HEIGHT)):
        neighbors.append((pixel[0] - 1, pixel[1] - 1))
    if (pixel[0] - 1 in range(DIGIT_DATUM_WIDTH) and pixel[1] + 1 in range(DIGIT_DATUM_HEIGHT)):
        neighbors.append((pixel[0] - 1, pixel[1] + 1))
    if (pixel[0] + 1 in range(DIGIT_DATUM_WIDTH) and pixel[1] - 1 in range(DIGIT_DATUM_HEIGHT)):
        neighbors.append((pixel[0] + 1, pixel[1] - 1))
    ...
    for neighbor in neighbors:
        if datum.getPixel(neighbor[0], neighbor[1]) == 0:
            visiter(neighbor, visited, datum)
    else:
        return
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    features['ateFood'] = state.getNumFood()-successor.getNumFood()
    prevFood = state.getFood().asList()
    prevDist = float("inf")
    for f in prevFood:
        dist = util.manhattanDistance(f, state.getPacmanPosition())
        if dist<prevDist:
            prevDist = dist
    currFood = successor.getFood().asList()
    currDist = float("inf")
    for f in currFood:
        dist = util.manhattanDistance(f, successor.getPacmanPosition())
        if dist<currDist:
            currDist = dist
    if currDist<prevDist:
        features['moveTowardsFood'] = 1
    else:
        features['moveTowardsFood'] = 0
    if state.getPacmanPosition() != successor.getPacmanPosition():
        features['movedPosition'] = 1
    else:
        features['movedPosition'] = 0
    prevGhosts = state.getGhostPositions()
    prevDist = float("inf")
    for g in prevGhosts:
        dist = util.manhattanDistance(g, state.getPacmanPosition())
        if dist<prevDist:
            prevDist = dist
    currGhosts = successor.getGhostPositions()
    currDist = float("inf")
    for g in currGhosts:
        dist = util.manhattanDistance(g, successor.getPacmanPosition())
        if dist<currDist:
            currDist = dist
    if currDist<prevDist:
        features['moveAwayFromGhost'] = 0
    else:
        features['moveAwayFromGhost'] = 1
    features['ateCapsule'] = len(state.getCapsules()) - len(successor.getCapsules())
    return features
