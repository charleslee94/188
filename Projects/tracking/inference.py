    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for x in newPostDist.keys():
                updatedBeliefs[x] += newPostDist[x]*self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for x in xrange(0, self.numParticles):
            particles = self.legalPositions[x % len(self.legalPositions)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                approx = emissionModel[util.manhattanDistance(particle, pacmanPosition)]
                weights[particle] += approx
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(0, self.numParticles-1):
            self.particles.append(temp[i%len(temp)])
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    temp*= emissionModels[i][util.manhattanDistance(pacmanPosition, particle[i])]
            weights[particle] += temp
        if weights.totalCount() == 0:
            self.initializeParticles()
        else:
            self.particles = []
            temp = util.nSample(weights.values(), weights.keys(), self.numParticles)
            for t in temp:
