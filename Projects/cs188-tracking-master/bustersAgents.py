...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
            mostLikely.append(i.argMax()) # get most likely position of each living ghost

        actionMap = util.Counter() # maps actions to minimum distance to ghost resulting from taking action
        for i in legal:
            actionMap[i] = 9223372036854775807 # instantiate values of actionMap to infinity
        for a in actionMap.keys():
            successorPosition = Actions.getSuccessor(pacmanPosition, a) # position of pacman after taking action a
            for p in mostLikely:
                mazeDist = self.distancer.getDistance(successorPosition, p)
                if mazeDist < actionMap[a]: # update actionMap if new min found
                    actionMap[a] = mazeDist
        for a in actionMap.keys():
            actionMap[a] = actionMap[a] * -1 # invert values for argMax()


        return actionMap.argMax()
